    ...
    var isComplete: Bool {
        ...
        let gradedIndices = Set(grades.compactMap { grade -> Int? in
            guard queueWordRecordIDs.indices.contains(grade.queueIndex),
                  queueWordRecordIDs[grade.queueIndex] == grade.wordRecordID else { return nil }
            return grade.queueIndex
        })
        return gradedIndices.count == queueWordRecordIDs.count
    }
...
enum PracticeSessionStore {
    ...
    static func save(_ session: SavedPracticeSession) {
        // Closing a summary or leaving the view must not recreate a completed checkpoint.
        guard !session.isComplete else {
            clear(sourceKind: session.resolvedSourceKind, setRecordID: session.setRecordID)
            return
        }
